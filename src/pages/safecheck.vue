<template>
  <div>
    <div class="tools">
      ...
      <el-input v-model="keyword" placeholder="主题" size="small" class="search-input"></el-input>
  ...
  methods: {
    async getList() {
      ...
      let {data} = await this.$http({
        url: 'safeCheckRecord/getSafeCheckRecordList',
        params: {
          companyId: 1,
    ...
    async delectClick(val) {
      let {data} = await this.$http({
        method: 'post',
        url: 'safeCheckRecord/deleteSafeCheckRecord',
        data: {
          ids: val
        }
      })
